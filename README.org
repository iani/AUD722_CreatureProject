...
The Creature class looks for audio files in the folder =CreatureAudioFiles= under the root directory of the AUD722_CreatureProject repository. This is where we put all audio files in order for them to be loaded by the Creature class when the server boots. 
