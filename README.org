...
The Creature library boots with the SuperCollider server, loads synthdefs, and loads audio buffers for all Creature subclasses. When you trigger a state (e.g., =day=), the creature performs a matching method or an action function defined in its actions dictionary. If a Creature's audio file is missing, it falls back to the default buffer (=cricket.wav=).

* How to use this folder and make your own Creature.
...
#+begin_src sclang
	*fileName { // tell your creature to use file basssdrum.wav
		// the file must be in the CreatureAudioFiles folder, together with all other audio files.
		^"bassdrum.wav";
	}
#+end_src

*Put your audio file in folder CreatureAudioFiles in your own repository.*

*** 1.2 Creature Project Audio files folder

The Creature class looks for audio files in the folder =CreatureAudioFiles= under the root directory of the AUD722_CreatureProject repository. This is where we put all audio files in order for them to be loaded by the Creature class when the server boots. If a Creature's file is missing, it will use the default buffer (=cricket.wav=).

** 2. Prepare your classes

*** 2.1. Put the CreatureProject Library code inside your SuperCollider Extensions folder and recompile
...
*** 3.1 Create a repository for your class

 Create a repository for your class inside the Extensions directory.

*** 3.2 Define your creature as a subclass of Creature.

Important: To understand how the methods in your class work, you should read the first section in the present file: [[*How this Library works][How this Library works]].
